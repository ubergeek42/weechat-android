...
class OnBackAnimationAwareDrawerLayout @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null)
    ...
    private fun registerUnregisterOnBackAnimationCallback(drawerOpen: Boolean) {
        if (drawerOpen) {
    ...
    init {
        addDrawerListener(object : DrawerListener {
            ...
            override fun onDrawerOpened(drawerView: View) {
                registerUnregisterOnBackAnimationCallback(true)
            }

            override fun onDrawerClosed(drawerView: View) {
                registerUnregisterOnBackAnimationCallback(false)
            }
        })
    }
}
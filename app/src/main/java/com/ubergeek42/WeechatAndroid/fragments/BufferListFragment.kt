...
class BufferListFragment : Fragment(), BufferListEye {
    ...
    private var visibleHotBufferIds: Collection<Long> = emptyList()
    ...
    @AnyThread @Cat override fun onBuffersChanged() {
        ...
        main {
            val newVisibleHotBufferIds = adapter.findAllVisibleHotBufferIds()

            if (weechatActivity.isBufferListVisible()) {
                showHideArrows()
            } else {
                val oneNewHotBufferId = newVisibleHotBufferIds.firstOrNull { it !in visibleHotBufferIds }
            ...
            visibleHotBufferIds = newVisibleHotBufferIds
        }
    }
    ...
    private fun findNextHotBufferPositionAboveVisibleBuffersOrNull(): Int? {
        ...
        return adapter.findNextVisibleHotBufferPositionOrNull(positionsToSearch)
    }

...
public class RelayServiceBinder extends Binder {
	...
	public void subscribeBuffer(String bufferPointer) {
		...
		if (buf.getLines().size() < Buffer.MAXLINES)
			relayService.relayConnection.sendMsg("(listlines_reverse) hdata buffer:"+bufferPointer+"/own_lines/last_line(-" + Buffer.MAXLINES + ")/data date,displayed,prefix,message,highlight,tags_array");
